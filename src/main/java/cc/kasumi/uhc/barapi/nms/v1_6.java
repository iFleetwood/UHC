...
public class v1_6 extends FakeDragon {
	...
	public Object getSpawnPacket() {
		Class<?> mob_class = ReflectionUtil.getCraftClass("Packet24MobSpawn");
		Object mobPacket = null;
		try {
			mobPacket = mob_class.newInstance();

			Field a = ReflectionUtil.getField(mob_class, "a");
			a.setAccessible(true);
			a.set(mobPacket, EntityID);// Entity ID
			Field b = ReflectionUtil.getField(mob_class, "b");
			b.setAccessible(true);
			b.set(mobPacket, EntityType.ENDER_DRAGON.getTypeId());// Mob type
																	// (ID: 64)
			Field c = ReflectionUtil.getField(mob_class, "c");
			c.setAccessible(true);
			c.set(mobPacket, getX());// X position
			Field d = ReflectionUtil.getField(mob_class, "d");
			d.setAccessible(true);
			d.set(mobPacket, getY());// Y position
			Field e = ReflectionUtil.getField(mob_class, "e");
			e.setAccessible(true);
			e.set(mobPacket, getZ());// Z position
			Field f = ReflectionUtil.getField(mob_class, "f");
			f.setAccessible(true);
			f.set(mobPacket, (byte) ((int) (getPitch() * 256.0F / 360.0F)));// Pitch
			Field g = ReflectionUtil.getField(mob_class, "g");
			g.setAccessible(true);
			g.set(mobPacket, (byte) ((int) 0));// Head
												// Pitch
			Field h = ReflectionUtil.getField(mob_class, "h");
			h.setAccessible(true);
			h.set(mobPacket, (byte) ((int) (getYaw() * 256.0F / 360.0F)));// Yaw
			Field i = ReflectionUtil.getField(mob_class, "i");
			i.setAccessible(true);
			i.set(mobPacket, getXvel());// X velocity
			Field j = ReflectionUtil.getField(mob_class, "j");
			j.setAccessible(true);
			j.set(mobPacket, getYvel());// Y velocity
			Field k = ReflectionUtil.getField(mob_class, "k");
			k.setAccessible(true);
			k.set(mobPacket, getZvel());// Z velocity

			Object watcher = getWatcher();
			Field t = ReflectionUtil.getField(mob_class, "t");
	...
	public Object getDestroyPacket() {
		Class<?> packet_class = ReflectionUtil.getCraftClass("Packet29DestroyEntity");
		Object packet = null;
		try {
			packet = packet_class.newInstance();

			Field a = ReflectionUtil.getField(packet_class, "a");
			a.setAccessible(true);
	...
	public Object getMetaPacket(Object watcher) {
		Class<?> packet_class = ReflectionUtil.getCraftClass("Packet40EntityMetadata");
		Object packet = null;
		try {
			packet = packet_class.newInstance();
			...
			Field b = ReflectionUtil.getField(packet_class, "b");
			b.setAccessible(true);
	...
	public Object getTeleportPacket(Location loc) {
		Class<?> packet_class = ReflectionUtil.getCraftClass("Packet34EntityTeleport");
		Object packet = null;
		try {
			packet = packet_class.newInstance();
			...
			Field c = ReflectionUtil.getField(packet_class, "c");
			c.setAccessible(true);
			c.set(packet, (int) Math.floor(loc.getY() * 32.0D));
			Field d = ReflectionUtil.getField(packet_class, "d");
			d.setAccessible(true);
			d.set(packet, (int) Math.floor(loc.getZ() * 32.0D));
			Field e = ReflectionUtil.getField(packet_class, "e");
			e.setAccessible(true);
			e.set(packet, (byte) ((int) (loc.getYaw() * 256.0F / 360.0F)));
			Field f = ReflectionUtil.getField(packet_class, "f");
			f.setAccessible(true);
	...
	public Object getWatcher() {
		Class<?> watcher_class = ReflectionUtil.getCraftClass("DataWatcher");
		Object watcher = null;
		try {
			watcher = watcher_class.newInstance();

			Method a = ReflectionUtil.getMethod(watcher_class, "a", new Class<?>[] { int.class, Object.class });
			a.setAccessible(true);
